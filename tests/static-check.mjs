...
for (const token of [
  "Godrej Agrovet Oil Palm Portfolio | PalmWatch",
  "Godrej Agrovet logo",
  ...
  assert.ok(html.includes(token), `Missing standalone Godrej contract: ${token}`);
}
...
for (const forbidden of [
  ...
  assert.ok(!html.includes(forbidden), `Standalone Godrej site leaked another company contract: ${forbidden}`);
}

assert.match(html, /let state = \{companyId:"gavl"/, "Godrej must be the fixed startup portfolio");
...
assert.match(html, /<header class="topbar">[\s\S]*?godrej-agrovet\.png[\s\S]*?alt="Godrej Agrovet logo"/, "Top branding must contain the approved accessible Godrej logo");
assert.ok(existsSync(new URL("../assets/company-logos/godrej-agrovet.png", import.meta.url)), "Approved Godrej logo must be bundled");
assert.ok(!existsSync(new URL("../assets/company-logos/nbl.png", import.meta.url)), "NBL logo must not be bundled");
...
assert.equal(packageJson.name, "godrej-agrovet-oil-palm-portfolio");
...
assert.ok(readme.includes("https://vjk7989.github.io/Godrej-Agrovet-Oil-Palm-Portfolio/"), "README must document the public repository URL");
...
console.log("Standalone Godrej static checks passed (company isolation, direct entry, branding, disclaimer, and Pages paths)." );
